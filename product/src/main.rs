...
fn main() {
    ...
    if matches!(sub, None | Some("gui")) {
        platform_input::hide_console_for_gui();
    }
    ...
    match sub {
        ...
        Some("ui-inspect") => {
            let code = run_ui_inspect(config, &args[2..]);
        ...
        Some(_) => {
            ...
            let code = run_cli(&mut service, &paths, &args[1..]);
            std::process::exit(code);
        }
    }
}
...
fn run_ui_inspect(config: config::AppConfig, args: &[String]) -> i32 {
        ...
        match args[i].as_str() {
            "--out" => {
                ...
                match args.get(i) {
                    ...
                    None => {
                        eprintln!("facial ui-inspect: --out requires a value");
                        return 1;
                    }
                }
            }
            "--tab" => {
                ...
                match args.get(i).and_then(|v| ui::Tab::from_vocab(v)) {
                    Some(t) => tabs.push(t),
                    None => {
                        eprintln!("facial ui-inspect: --tab requires a valid vocab (project|quality_iq|identity|duplicates|run_debug|manual|media|compare|lanes|options)");
                        return 1;
                    }
                }
            }
            other => {
                eprintln!("facial ui-inspect: unknown flag {other}");
                return 1;
            }
        }
        i += 1;
    ...
    match ui_inspect::run(config, out_dir, &tabs) {
        ...
        Err(e) => {
            eprintln!("facial ui-inspect failed: {e}");
            1
        }
    }
}
...
fn run_cli(service: &mut FacialService, paths: &ApiPaths, args: &[String]) -> i32 {
    let Some(sub) = args.first().map(|s| s.as_str()) else {
        eprintln!("facial: missing subcommand");
        print_cli_usage();
        return 1;
    };
    let rest = &args[1..];
...
/// `facial command <path>` | `facial command --json '<json>'`
